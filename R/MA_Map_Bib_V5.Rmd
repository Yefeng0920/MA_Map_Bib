...
If you wish to use the provided `R` code to analyze your own data, we recommend that you carefully read the functions and packages provided below. In other words, if you want to report this useful data and add values in a meta-analysis, you need to spend enough time to follow the instructions in this web page (yes, there is no such thing as a free lunch). Basically, you need to have some experience writing R code. If you have spent enough time and still get stuck, you can look at the end of this page where we provide provide instructions on how to generalize and translate the example `R` script to your own data leveraging the Large Language Model (LLM).
...
# Leveraing the Large Language Model to make your own figures

For readers who are not able to generalize and translate the example `R` script to their own data, we provide instructions on how to use a Large Language Model (LLM) to generalize and translate the example `R` script to your own data.

Specifically, we will use the exercise data, (`Data_exercise1`) @bodey2018phylogenetically and (`Data_exercise2`)@yang2023ALAN for illustration, and show how to use three LLMs  (Open AI `ChatGPT`, Google `Gemini`, and Microsoft `Copilot`) to draw the main enrichment figures shown above. 
...
Then, let's try to have conversation with ChatGPT 3.5 as follows:
...
The ChatGPT 3.5 will return you the adapted `R` code:

```{r Figure I ChatGPT, warning = FALSE, echo = TRUE, fig.height=10, fig.width=10}
# R code from ChatGPT 3.5
...
Nice! You get the grid-like plot where you can identify knowledge gaps by looking at the size of the bubble, thus setting the agenda for future research to fill such gaps. Certainly, you can make changes to the code and improve the figure according to your own preference, including adjusting the colors, labels, and other plot properties as needed to customize the plot according to your preferences.

### Alluvial diagrams

With the same data (`dat`), we next teach you how to use ChatGPT 3.5 to make an alluvial diagram.

Starts your dialogue with ChatGPT 3.5 as:
...
Now, I want to apply this example R code to my own data named `dat`. The `dat` has two variables named `Flight`, `Sexes`, `Ages`, `Tag.Type`. Can you use the example R code with my own data `dat` by replacing the variables shown in the example code with the variables in my our data? Note that the function `dlong()`, `sankey_p()`, and `sankey_p_label()` are custom functions.
</pre>

```{r Figure II ChatGPT, warning = FALSE, echo = TRUE, fig.height=5, fig.width=8}

# code returned by ChatGPT 3.5
library(ggplot2)
...
We will use the bibliometric data associated with the `Data_exercise2_bib`@yang2023ALAN to illustrate how to construct a co-authorship network by "asking" ChatGPT 3.5 to adapt the provided example `R` code. We collected bibliographic data of the `Data_exercise2_bib`@yang2023ALAN from Scopus using DOI-based searches.
...
Open your ChatGPT 3.5 GUI and type the prompt as:
...
```{r Figure IV ChatGPT, warning = FALSE, echo = TRUE, fig.height=8, fig.width=8}
# Load required libraries
library(igraph)
...
We will use the alternative metric data associated with the `altmetrics_exercise`@yang2023ALAN to illustrate how to conduct the altmetric analysis by "asking" ChatGPT 3.5 to adapt the provided example `R` code. The alternative data of the `altmetrics_exercise`@yang2023ALAN were collected from Altmetrics online service (www.altemetric.com). You can use the custom function provided above to automatically extract Altmetric score, policy, and patent citation for each study included in the meta-analytic evidence @yang2023ALAN. Alternatively, you can extract these statistics manually from Altmetrics online service (www.altemetric.com). Anyway, assume you have the `altmetrics_exercise`, which contains Altmetric score, policy, and patent citation.

```{r exercise data2 altmetric, warning = FALSE, message = FALSE, echo = TRUE}
# load altmetric data
...
```{r Figure VI ChatGPT, warning = FALSE, message = FALSE, echo = TRUE, fig.height=5, fig.width=5}
# Load packages
library(ggplot2)
library(ggbeeswarm)
